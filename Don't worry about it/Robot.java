...
public class Robot extends TimedRobot {
 ...
 //Station (starting position) and cone/cube
  ...
  public void robotInit() {
    ...
    //The distance in metres one revolution of the motor turns the wheel
    leftAEncoder.setPositionConversionFactor(0.05);
    ...
    try {
      ...
      SmartDashboard.putNumber("Top Width", 0.0);
      SmartDashboard.putNumber("Bottom Width", 0.0);
    }
    ...
    try {
      topCamera.setFPS(15);
      topCamera.setResolution(600, 400);
      SmartDashboard.putNumber("Top Width", 600);
      SmartDashboard.putNumber("Top Height", 400);
    } catch(VideoException e) {
      SmartDashboard.putNumber("Top Width", 0.0);
    ...
    iPromiseThisWontBreakAnything = new Thread(() -> {
      while(true) {
        ...
        if(experimentalVelocityConstant != 0.0) {
          SmartDashboard.putNumber("Left Speed", driveLeftA.get() * experimentalVelocityConstant);
          SmartDashboard.putNumber("Right Speed", driveRightA.get() * experimentalVelocityConstant);
        }
    ...
    autoSpeed.setPriority(Thread.MAX_PRIORITY);
    autoSpeed.setDaemon(true);
    autoSpeed.start();
  }
  ...
  public void autonomousInit() {
    ...
    SmartDashboard.putNumber("Auto Start", autoStart);
    ...
    for(double speed = 0; speed <= 0.4; speed += 0.025) {
      if(speed == 0.025) {
        ...
        experimentalVelocityConstant = ((0.1 * leftAEncoder.getPositionConversionFactor() / (Timer.getFPGATimestamp() - currTime)) / speed);
      } else {
        driveLeftA.set(speed);
        driveLeftB.follow(driveLeftA);
        driveRightA.set(speed);
  ...
  public void teleopPeriodic() {
    ...
    if(ps1.getCircleButtonPressed()) {
      SmartDashboard.putBoolean("Auto Drive", !SmartDashboard.getBoolean("Auto Drive", true));
      SmartDashboard.updateValues();

      if(SmartDashboard.getBoolean("Auto Drive", false)) {
        while(SmartDashboard.getBoolean("Deciding", true)) {
          try {
            Thread.sleep(50);
          } catch(InterruptedException e) {
        ...
        driveLeftA.set(SmartDashboard.getNumber("Target Left Speed", driveLeftA.get()));
        driveLeftB.follow(driveLeftA);
        driveLeftA.set(SmartDashboard.getNumber("Target Right Speed", driveLeftA.get()));
        driveLeftB.follow(driveLeftA);
      }
    }
